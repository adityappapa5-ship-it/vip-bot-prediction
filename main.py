...
def check_join(uid):
    """Bhai bina join kiye access nahi milega"""
    for c in CHANNELS:
        try:
...
def get_data_safe():
    """API Block Todne Ke Liye"""
    h = {'User-Agent': 'Mozilla/5.0', 'Origin': 'https://ar-lottery01.com'}
    try:
    ...
    while True:
        try:
            ...
            if curr_p != last_p:
                last_p = curr_p
                next_p = int(curr_p) + 1
                pred = "🌕 𝗕𝗜𝗚" if int(history[0]['number']) < 5 else "🌑 𝗦𝗠𝗔𝗟𝗟"
                ...
                # Wait for Result & Sticker
                time.sleep(55)
                res_data = get_data_safe()
