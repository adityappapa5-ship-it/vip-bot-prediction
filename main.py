...
def check_status(user_id):
    for c in CHANNELS:
        try:
            s = bot.get_chat_member(c, user_id).status
...
def start(message):
    if check_status(message.from_user.id):
        ...
        bot.send_message(message.chat.id, "💎 <b>ADITYA PAPA CLOUD ACTIVE</b> 💎\n24/7 Mode Unlocked!", parse_mode="HTML", reply_markup=markup)
    else:
        ...
        bot.send_message(message.chat.id, "❌ <b>ACCESS LOCKED</b>\nBhai, join channels to use bot!", parse_mode="HTML", reply_markup=markup)
...
def engine(message):
    ...
    while True:
        ...
        if current_p != last_p:
            ...
            time.sleep(55) # Game timing sync
